...
//	private String host;
	...
	public void setConsulApi(ConsulApi consulApi) {
		this.consulApi = consulApi;
	}
	
	public void clearConsulApi(ConsulApi consulApi) {
		this.consulApi = null;
	}
...
//			consulApi.cleanAttributes(location.getHost(),location.getPort());
	...
	private void bindServlet(String alias, Map<String, Object> servletSetting,NetworkLocation location) {
		String tags = (String) servletSetting.get("tags");
		
		try {
			if(!bindingIds.containsKey(alias)) {
				String id = consulApi.registerService(createServiceName(servletSetting), location.getHost(), location.getPort(), alias,servletSetting, tags);
